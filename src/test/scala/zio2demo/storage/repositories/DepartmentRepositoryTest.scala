...
object DepartmentRepositoryTest extends ZIOSpecDefault {
  ...
  case class ConnectionMock(id: String) extends Connection {
    def add[E <: Entity](value: E)(using entity: EntityType[E]): IO[ApplicationError, Unit] =
      value.id match
        case id: UUIDv7 if id.compareTo(department.id) == 0 => ZIO.succeed[Unit](())
    ...
    def get[E <: Entity: ClassTag](uuid: UUIDv7)(implicit entity: EntityType[E]): IO[ApplicationError, Option[E]] =
      uuid match
        case id: UUIDv7 if id.compareTo(department.id) == 0 => ZIO.succeed[Option[E]](department.asInstanceOf[E].pure[Option])
        case id: UUIDv7 if id.compareTo(uuid_2) == 0 => ZIO.succeed[Option[E]](None)
        case id: UUIDv7 if id.compareTo(uuid_3) == 0 => ZIO.fail[ApplicationError](NotFound(s"Department with id '${id.toString()}' has not been found! "))
        case _ => ZIO.fail[ApplicationError](BadRequest(s"Bad request has been sent!"))
